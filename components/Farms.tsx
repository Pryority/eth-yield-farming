          ...
          .map((farm: any, i:any) =>(
            <div key={i} className="flex relative p-2">
              ...
              >
                ...
                <div className={`${farm.name === "Rocket Pool" ? 'absolute text-orange-900' : "absolute"} ${farm.name === "Alpha Homora V2" ? 'absolute text-blue-900 max-w-[60%] sm:max-w-[70%]' : "absolute"}`}>
                  <h3 className="text-2xl font-bold">{farm.name}</h3>
                  <h2 className={`${farm.name === "Alpha Homora V2" ? "bg-blue-400 text-white":""} ${farm.name === "NFTx" ? "bg-red-400 text-white" : ""} ${farm.name === "Rocket Pool" ? "bg-orange-600 text-white":""} ${i > 2 ? "bg-slate-200" :""} text-xs p-1 rounded-lg w-fit`}>{farm.symbol}</h2>
                </div>
              </Link>
            </div>
          ))
